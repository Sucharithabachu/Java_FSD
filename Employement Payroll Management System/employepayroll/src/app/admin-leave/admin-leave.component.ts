...
export class AdminLeaveComponent implements OnInit {
  ...
  pending(){
    this.service.pendingLeave().subscribe((data: Leaves[]) => {
      this.leaves = data;
    })
  }

  remaining(){
    this.service.remainingLeave().subscribe((data: Leaves[]) => {
      this.leaves = data;
    })
  }
  ...
  approve(employeeid:string,id:number){
    ...
    this.service.updateLeave(id,this.leave).subscribe((data: any)=> {
      this.pending();
    })
  }
